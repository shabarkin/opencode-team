...
import { ForbiddenError, PermissionNotFoundError } from "../errors"
...
export const sessionHandlers = HttpApiBuilder.group(InstanceHttpApi, "session", (handlers) =>
  Effect.gen(function* () {
    ...
    const abort = Effect.fn("SessionHttpApi.abort")(function* (ctx: { params: { sessionID: SessionID } }) {
      const request = yield* HttpServerRequest.HttpServerRequest
      const team = Flag.OPENCODE_EXPERIMENTAL_AGENT_TEAMS
        ? yield* legacy(async () => {
            const { Team } = await import("@/team")
            return {
              Team,
              match: await Team.findBySession(ctx.params.sessionID),
            }
          })
        : undefined
      if (team?.match && request.headers["x-opencode-session"] !== ctx.params.sessionID) {
        return yield* new ForbiddenError({ message: "Forbidden" })
      }
      yield* promptSvc.cancel(ctx.params.sessionID)
      const match = team?.match
      if (team && match?.role === "lead") {
        yield* legacy(() => team.Team.cancelAllMembers(match.team.name)).pipe(
          Effect.catchCause((cause) =>
            Effect.logWarning("team cancel cascade failed").pipe(
