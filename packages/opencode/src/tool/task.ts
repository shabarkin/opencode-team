  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("TaskTool.execute")(function* (
      ...
      const linked = yield* Effect.promise(async () => {
        ...
        return item ? { parentTeam: item.parentTeam, parentMember: item.parentMember, mode: "task" as const } : undefined
      })
      ...
      if (session) {
        yield* sessions.setPermission({ sessionID: session.id, permission }).pipe(Effect.catchCause(() => Effect.void))
      }
      if (linked) {
