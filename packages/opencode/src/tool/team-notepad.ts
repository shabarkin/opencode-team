...
export const TeamNotepadTool = Tool.define("team_notepad", {
  description:
  ...
  async execute(params, ctx): Promise<{ title: string; output: string; metadata: Record<string, any> }> {
    ...
    switch (params.action) {
      case "list": {
        const data = await TeamNotepad.list(teamName)
      ...
      case "read": {
        ...
        const value = await TeamNotepad.read(teamName, params.key)
        if (value === undefined) {
      ...
      case "write": {
        ...
        await TeamNotepad.write(teamName, params.key, params.value)
        return {
