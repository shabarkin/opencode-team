...
:::note
OpenCode does not always expose the same file-editing surface to every model. Most models get `edit` and `write`, while supported OpenAI GPT models receive `apply_patch` instead.
:::
...
This tool applies a structured patch envelope to your codebase. Supported OpenAI GPT models receive this tool instead of `edit` and `write`.

Use it when the model needs to create, update, move, or delete files in one patch.
...
This fork adds experimental `team_*` tools for coordinated multi-agent work.
They are available only when `OPENCODE_EXPERIMENTAL_AGENT_TEAMS=1` is set.

- `team_create`, `team_spawn`, `team_shutdown_all`, `team_shutdown`, `team_merge`, and `team_cleanup` manage the team lifecycle.
- `team_message`, `team_reply`, `team_broadcast`, `team_inbox`, and `team_submit_result` handle coordination and reporting.
- `team_tasks`, `team_claim`, `team_collect`, `team_wait`, `team_notepad`, `team_phase`, and `team_status` track shared work and progress.
- `team_approve_plan`, `team_health`, `team_restart`, `team_delegate`, and `team_request_spawn` help with review, recovery, and extra delegation.

These tool IDs are still experimental and may change.

See [Agent Teams](/docs/agent-teams) for the full lifecycle and [Steering](/docs/steering) for review and redirect flows.
