...
In this fork, teammate sessions created in isolated worktree mode also add a session-level `external_directory` allow for their own worktree automatically. That keeps normal teammate edits and shell work inside the teammate checkout from repeatedly prompting for external-directory approval.
...
- `edit` — all file modifications (covers `edit`, `write`, `apply_patch`, `multiedit`)
