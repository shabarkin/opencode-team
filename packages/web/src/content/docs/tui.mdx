...
Toggle delegate mode for the active lead session.
...
:::note
Only available when Agent Teams are enabled and the current session is the team lead.
:::
...
With `OPENCODE_EXPERIMENTAL_AGENT_TEAMS=1`, lead sessions can coordinate teammates from the TUI.
The sidebar and footer show member status, task progress, and pending spawn requests.

- Use `session_child_first`, `session_child_cycle`, and `session_parent` to move between the lead and teammate sessions.
- Use `session_steer` (`ctrl+s` by default) to open the team steer dialog and send follow-up instructions to one teammate or all active teammates.
- Use `team_message` in the prompt when you want to send direct coordination messages.

This fork-specific workflow is experimental and may change.

See [Agent Teams](/docs/agent-teams) for the end-to-end model and [Steering](/docs/steering) for the TUI control flow.
