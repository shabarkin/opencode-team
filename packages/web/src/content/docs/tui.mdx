...
The header and sidebar show member status, task progress, and pending spawn requests.

- Use `session_child_first`, `session_child_cycle`, and `session_parent` to move between the lead and teammate sessions.
- Use the **Steer** action to send follow-up instructions to one teammate or all active teammates.
