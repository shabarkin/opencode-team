...
This fork adds an experimental Agent Teams workflow for coordinated multi-agent work.
Enable it with `OPENCODE_EXPERIMENTAL_AGENT_TEAMS=1`.

- Start with `team_create` to create a lead-owned team.
- Use `team_spawn` to add specialists and split work.
- Coordinate with `team_message`, `team_broadcast`, `team_tasks`, and `team_collect`.
- Use `team_approve_plan` when a teammate starts in plan-approval mode.
- Run `/delegate` when you want the lead to stay in coordination mode.

Each teammate runs in its own session, so you can move between the lead and workers from the TUI.

For the full lifecycle, architecture, safety model, and implementation map, see [Agent Teams](/docs/agent-teams), [Worktrees](/docs/worktrees), and [Steering](/docs/steering).
